...
- [x] `locus agent report|doctor|setup` (fail-closed MCP config merges — malformed configs left byte-for-byte untouched; post-write registration-probe verification on `--apply`)
- [x] MCP resources (`locus://session|doctor|bindings`) + `locus_context` prompt
- [x] Tool description pin tags; `initialize.instructions`
- [x] MCP auto-pin knobs from workspace / `LOCUS_AUTO_PIN` (advisory-only: server never self-pins; honest `session.auto_pin_denied` audit — operator-delegation design pending)
- [x] Fail-closed MCP session pin-anchoring (`pin_changed`) — every MCP session (stdio process / HTTP `Mcp-Session-Id` / stateless process scope) anchors to the initialize-time binding identity (`crates/locus-mcp/src/anchor.rs`); cross-alias re-pin under a live session returns a structured `pin_changed` error + `safe_next.action=reinitialize_client` instead of silently adopting the new identity; `locus_verify_session` forces `session_ok=false` on mismatch; anchor survives `leave`, persists in the HTTP session disk format (cross-process), audits `mcp.anchor_*` — closes the wrong-account pin-swap gap
- [x] Operator onboarding polish — guided `locus client add` (validators + provider menu + `CredentialRef::validate`; raw secrets never accepted), `--ttl` auto-leave on `enter`/`pin` (`policy.default_ttl` / `max_ttl` clamp + `pin_expiring` doctor warn), Grok Build write path (`~/.grok/config.toml` fail-closed `toml_edit` merge; joins `--client all` + registration probe), Claude user scope via the `claude` CLI (`--claude-scope user`) — [docs/onboarding.md](./docs/onboarding.md)
- [x] Control-capability onboarding + posture — `init`/`quickstart` mint + persist `LOCUS_CONTROL_CAPABILITY` (0600; opt-out `--no-persist-capability`), `locus capability status|persist|unpersist`, shell hook export, doctor findings incl. INFO `control_capability_persisted` (never escalates the verdict); the agent boundary stays the MCP surface, not the CLI (SECURITY.md § Control-plane authority boundary)
...
- [x] Dual-IDE dogfood matrix (no secrets) — `scripts/dogfood-dual-ide.sh` dry-runs setup per found client (claude/cursor), validates MCP config path contains `locus` without printing values, optional multi-account walk when personal+client aliases set, prints matrix (found / locus_reg / multi_account); soft-skip missing; `LOCUS_DOGFOOD_REQUIRE_DUAL=1` hard-fails
- [ ] Dogfood: agent report `ready` on real Claude Code + Cursor installs (personal + client) — dual-IDE matrix + multi-account scripts landed; **live dual-IDE UI** (both apps open, catalogs reload) still operator-manual
...
- [x] Firm soft-warn on hub doctor/readiness — hub [PR #277](https://github.com/ashlrai/ashlr-hub/pull/277) (`checkLocusFirm`: enrolled>0 + locus CLI available + `locus.firm` not true → non-blocking `locus-firm` warn; **never hard-blocks mutate**; monorepo/0-enrolled stays quiet). Production fleet checklist: hub `docs/LOCUS-FIRM-FLEET.md`. Hub-only — do **not** port into monorepo `locus.ts`
- [ ] Always-on firm-mode enforce by default (still **off** unless `locus.firm` / `locus.enforce` / `LOCUS_ENFORCE` set — #258 lands opt-in firm profile; #277 soft-warns only; do not flip monorepo default until pin is guaranteed on all hub paths)
...
- [x] `locus verify session` → doctor + whoami + safe_next JSON pack for hub (`session_ok`); MCP `locus_verify_session` (same pack, available unpinned) — MCP doctor sites (`locus://doctor`, `/mcp/sse` ticks) run real external probes via shared `gather_doctor_external`, no hardcoded conservative failure
...
- [x] Adapter registry ed25519: `ed25519:<base64>` verify (+ HMAC-SHA256 backcompat); `LOCUS_ADAPTER_TRUST_KEYS` env / test fixtures; `--require-signed` accepts either scheme when key trusted
- [x] Adapter registry production trust store: `$LOCUS_HOME/trust/adapter-keys.toml` (0600) + `locus adapter trust list|add --ed25519-pub`; verify merges file + env (env wins on same id). Signed release manifests / published registry root still open
...
- [x] Streamable HTTP / remote multiplexor (platform phase): streamable-HTTP-lite on `locus-mcp --http` with `GET /mcp` capabilities (tool **names** + pin summary, values-free), Accept/`Content-Type` negotiation (`application/json` preferred; single-event SSE when small; **multi-message SSE** for large `tools/call` / bodies — progress + optional text chunks + final JSON-RPC), `GET /mcp/sse` session_ok hub ticks (`locus.session_tick`), **`Mcp-Session-Id` + cross-process resume LANDED** ([#31](https://github.com/ashlrai/locus/pull/31): in-memory cache + **file-backed resume** under `$LOCUS_HOME/http-sessions/` or `LOCUS_MCP_SESSION_DIR`; 30m idle TTL, max N, mint on initialize/first POST, load-on-miss, unknown/corrupt → 404 fail-closed, `DELETE /mcp` teardown memory+disk; no secrets on disk — id/timestamps/optional pin summary only; no redis), **pre-auth hardening** (413 above 8 MB bodies, 431 header caps, JSON-RPC `-32700` on unparseable bodies, session mint on `initialize` only — garbage POSTs cannot exhaust session capacity), per-session + process-level pin-swap anchoring (`anchor_ok` on `GET /mcp`; anchored session records resume from disk), remote-deploy docs (reverse proxy, `LOCUS_MCP_HTTP_TOKEN`, `LOCUS_HOME`, pin-before-serve, forward `Mcp-Session-Id`), HTTP unit/integration tests (health + auth fail-closed + Accept 406/SSE multi + session mint/reuse/reject + session SSE + disk resume/expire). **Multi-tenant remote multiplexor LANDED**: `locus-mcp --http --multi-tenant` — per-request tenant routing via operator-minted grants (`locus mcp mint|list|revoke`; sealed ci-backed sessions + `lmt_` bearer tokens whose HMAC-only lives in `$LOCUS_HOME/mcp-grants/`), `X-Locus-Tenant-Token` on every request, per-grant catalogs/whoami/drift/SSE, hard `http-sessions-mt/` partition, 401 `invalid_grant`/403 `tenant_mismatch` fail-closed, no agent pinning (`tenant_fixed_by_grant`), per-grant session caps + worker teardown on last-session death
...
| M4 Hub composition | in progress |
| M5 Verification plane | mostly done (claim+session verify incl. MCP `locus_verify_session` + real external probes, continuous `watch` heartbeat, e2e/dogfood coverage, fail-closed macOS worker sandbox, Linux bwrap/path sandbox partial, conformance CI, audit webhook sink, adapter registry v0 + ed25519 + production trust store, HTTP session SSE + file-backed resume #31 + pre-auth hardening + pin-swap session anchoring, multi-tenant remote multiplexor via operator-minted grants `locus mcp mint\|list\|revoke`; still open: signed release manifests / published registry root, full seccomp/VM sandbox) |
