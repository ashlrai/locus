...
| M4 Hub composition | in progress |
| M5 Verification plane | partial (claim+session verify incl. MCP `locus_verify_session` + real external probes, continuous `watch` heartbeat, e2e/dogfood coverage, fail-closed macOS worker sandbox, Linux bwrap/path sandbox partial, conformance CI, audit webhook sink, adapter registry v0 + ed25519 + production trust store, HTTP session SSE + file-backed resume #31 + pre-auth hardening + pin-swap session anchoring; still open: multi-tenant remote multiplexor) |
