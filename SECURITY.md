...
- **Malicious code inside a worker that already holds that binding’s credentials** — blast radius is intentionally one binding. Optional OS sandbox (`LOCUS_WORKER_SANDBOX=1`) is additive best-effort (macOS Seatbelt; Linux bubblewrap or path-only fallback tagged `path` — not a VM). Network stays allowed by default for MCP; opt in to deny with `LOCUS_WORKER_SANDBOX_NO_NETWORK=1` / `upstream.sandbox_no_network` (bwrap `--unshare-net`; Seatbelt best-effort). See [docs/workers.md](./docs/workers.md).
